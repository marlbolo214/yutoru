...
assert.doesNotMatch(html,/Standard DEMO/);assert.match(html,/YUTORU Standard/);assert.match(html,/standard\.js/);
...
assert.equal(core.seed().store.adminPin,'1234','管理者PIN初期値が不正です');
assert.equal(core.seed().store.accountingPin,'5678','経理PIN初期値が不正です');
assert.match(js,/accountingPin/,'経理PINの保存設定がありません');
assert.match(js,/role==='manager'.*adminPin.*accountingPin/s,'管理者PINと経理PINが分離されていません');
assert.match(js,/function requestRole\(role\)/,'管理者・経理PIN認証がありません');
assert.match(js,/x\.role=null/,'再読み込み時に権限選択へ戻りません');
assert.match(js,/PINが違います/,'PINエラー案内がありません');
